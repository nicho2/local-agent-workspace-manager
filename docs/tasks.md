...
## [x] T029 - Ajouter suppression controlee des workspaces, policies et agents
...
## [x] T031 - Permettre l'execution planifiee reelle avec garde-fous explicites
...
Note de realisation : 2026-04-28 - Ajout du champ explicite `execution_mode` (`dry_run` par defaut, `real_execution` sur opt-in) sur les schedules API/UI, avec migration SQLite pour compatibilite des bases existantes. Le worker propage maintenant ce mode vers `RunCreate.dry_run` tout en conservant `trigger=schedule` et `requested_by=schedule-worker`; les blocages setting global/policy restent traces en runs `blocked` avec `dry_run=false` quand un schedule reel est du. La page Schedules affiche le mode d'execution, ajoute une confirmation explicite avant sauvegarde d'un schedule en execution reelle et rappelle les garde-fous. Tests backend/frontend et documentation (spec, architecture, README, backlog) mis a jour. Validation : `cd apps/api && pytest` ; `cd apps/web && npm test` ; `cd apps/web && npm run build`.
